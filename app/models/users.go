...
func FindAllContacts(id gocql.UUID) page{
	...
	Session, _ := cluster.CreateSession()
	defer Session.Close()
	 iter := Session.Query("SELECT contact_id, contact_name, contact_phone FROM contacts_by_user WHERE user_id=? ",id).Iter();
	var b Contact
	for iter.Scan(&b.Pk,&b.ContactName,&b.ContactPhone) {
...
func UpdateContactInfo(contact Contact){
	...
	session, _ := cluster.CreateSession()
	defer session.Close()
	err:= session.Query("UPDATE contacts_by_user SET contact_name=?,contact_phone=? WHERE user_id=? and contact_id=? and added_time=(SELECT added_time  FROM contacts_by_user WHERE user_id=? and contact_id=? ALLOW FILTERING)",contact.ContactName,contact.ContactPhone,contact.UserID,contact.Pk,contact.UserID,contact.Pk).Exec();
	checkErr(err)
}
